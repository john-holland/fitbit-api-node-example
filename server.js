...
router.get("/authorize", function (req, res) {
    ...
    res.redirect(client.getAuthorizeUrl('activity heartrate location nutrition profile settings sleep social weight','https://fitbit-oauth-example-john-holland.c9users.io/callback'));
});

// handle the callback from the Fitbit authorization flow
router.get("/callback", function (req, res) {
    // exchange the authorization code we just received for an access token
    client.getAccessToken(req.query.code, 'https://fitbit-oauth-example-john-holland.c9users.io/callback').then(function (result) {
